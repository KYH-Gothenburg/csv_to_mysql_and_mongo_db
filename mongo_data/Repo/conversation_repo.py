...
def store_conversations(conversations, lines):
    for conv_no, conversation in enumerate(conversations):
        print(f'Processing conversation {conv_no} of total {len(conversations)}')
        ...
        conversatiion_obj = Conversation(movie_id=movie_id, conversation_lines=conversation_lines)
        conversatiion_obj.save()